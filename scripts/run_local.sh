...
INSERT INTO accounts (id, name, currency, opening_balance_paise, min_threshold_paise)
VALUES
  ('ACC-001', 'Operating Account', 'INR', 5000000000, 2000000000),
  ('ACC-002', 'Payroll Account',   'INR', 2000000000,  500000000),
  ('ACC-003', 'Reserve Account',   'INR', 8000000000, 3000000000)
...
with Session() as db:
    ...
    batch_row = db.execute(
        text("INSERT INTO ingestion_batches (account_id, source, status, created_at) VALUES ('ACC-001', 'local', 'processing', now()) RETURNING id")
    ).fetchone()
    db.commit()
    batch_id = batch_row.id
    print(f"  batch_id={batch_id}")

    # Load CSVs for all accounts
    ...
    for acct in ACCOUNTS:
        ...
        if alert:
            print(f"  ALERT [{alert['severity'].upper()}] {acct}: breach on {alert['breach_date']}, shortfall ₹{alert['shortfall_paise']/100:,.0f}")
        else:
    ...
    db.execute(text("UPDATE ingestion_batches SET status='done' WHERE id=:bid"), {"bid": batch_id})
    db.commit()
...
docker exec -i localstack-postgres-1 psql -U postgres -d recon <<'SQL'
SELECT 'bank_statement_lines' AS tbl, COUNT(*) FROM bank_statement_lines
